...
const uint32_t pipeAddress = 111156789; // адрес рабочей трубы;
byte data[1];
...
void setup() {
  ...
  radio.setPALevel(RF24_PA_LOW); // скорость обмена данными RF24_1MBPS или RF24_2MBPS
  radio.setCRCLength(RF24_CRC_8); // размер контрольной суммы 8 bit или 16 bit
  radio.setChannel(0x6f);         // установка канала
  radio.setAutoAck(false);       // автоответ
  radio.openReadingPipe(1, pipeAddress); // открыть трубу на приём
